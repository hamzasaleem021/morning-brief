...
(function() {
  ...
  var isOpen = false;
  var isAdding = false; // Prevent rapid-fire adding
...
@media (max-width: 480px) {
  ...
  function renderCategories(filterQuery = '') {
    ...
    var categories = Object.entries(discoveryData.categories);
  ...
  function updateStats() {
    ...
    var categoryCount = Object.keys(discoveryData.categories).length;
    var sourceCount = discoveryData.source_count || 0;
    var yourCount = typeof State !== 'undefined' ? State.getCount() : 0;
    ...
    var yourStat = document.getElementById('stat-yours');
  ...
  function updateSourceButton(url, state) {
    var card = document.querySelector(`[data-source-url="${url}"]`);
    if (!card) return;
  ...
  function disableAllAddButtons() {
    var buttons = document.querySelectorAll('.add-btn:not(.added)');
    buttons.forEach(btn => {
      btn.disabled = true;
